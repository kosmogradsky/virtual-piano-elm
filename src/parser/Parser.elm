...
type Parser raw result = Parser (raw -> Result String (result, raw))

pop : Int -> Parser (List a) (List a)
pop n = Parser (\s -> Ok (List.take n s, List.drop n s))
...
return v = Parser <| \raw -> Ok (v, raw)

empty : String -> Parser raw a
empty msg = Parser <| \_ -> Err msg

get : Parser raw raw
get = Parser <| \raw -> Ok (raw, raw)

run : Parser raw a -> raw -> Result String (a, raw)
run (Parser parse) raw = parse raw